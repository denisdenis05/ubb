...
class Services:
    ...
    def removeAllStudentGrades(self, studentId):
        ...
        for gradeIdentifier in allStudentGrades:
            removedGrades = self.removeStudentsGradesAtADiscipline(gradeIdentifier)
            if removedGrades is not None:
                for removedGrade in removedGrades:
                    gradeIdentifier = (removedGrade.getStudentId(), removedGrade.getDisciplineId())
                    if gradeIdentifier in allRemovedGrades:
                        allRemovedGrades[gradeIdentifier].append(removedGrade)
                    else:
                        allRemovedGrades[gradeIdentifier] = [removedGrade]
        return allRemovedGrades
    ...
    def updateStudent(self, studentId: int, newStudentName: str):
        if self.__studentsRepository.getItem(studentId) is not None:
            try:
                studentToUpdate = self.__studentsRepository.getItem(studentId)
    ...
    def updateDiscipline(self, disciplineId: int, newDisciplineName: str):
        if self.__disciplinesRepository.getItem(disciplineId) is not None:
            try:
                disciplineToUpdate = self.__disciplinesRepository.getItem(disciplineId)
                newDiscipline = Discipline(disciplineId, newDisciplineName)
                self.__disciplinesRepository.updateItem(disciplineId, newDiscipline)
    ...
    def getStudent(self, studentId):
        return self.__studentsRepository.getItem(studentId)

    def getDiscipline(self, disciplineId):
        return self.__disciplinesRepository.getItem(disciplineId)
